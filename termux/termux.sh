...
# wraps the binaries to unset LD_PRELOAD and TERMUX_EXEC__PROC_SELF_EXE
...
# for bin in aidl aapt aapt2 dexdump zipalign split-select; do
#     if [ -f "$TOOLS/$bin" ] && [ ! -f "$TOOLS/${bin}.bin" ]; then
#         mv "$TOOLS/$bin" "$TOOLS/${bin}.bin"
#         cat > "$TOOLS/$bin" << EOF
# #!/data/data/com.termux/files/usr/bin/bash
# unset LD_PRELOAD
# unset TERMUX_EXEC__PROC_SELF_EXE
# exec "$TOOLS/${bin}.bin" "\$@"
# EOF
#         chmod +x "$TOOLS/$bin"
#         echo "patched: $bin"
...
echo "and either install the required cmake version in termux, or remove the version declaration from your app-level gradle.kts"
echo
